...
public:
        ...
        image_transport_(node_handle)
    {
        ...
        //test_pub_ = node_handle.advertise<std_msgs::Bool>("/test_pub",1);
    }
    ...
    void set_image(void)
    {
        try
        {
                if (bgra32Frame != NULL)
                {
                    ...
                    try
                    {
                        //test_pub_.publish(std_msgs::Bool());
...
//                        std::cout << msec << " ms = " << 1000/msec << " hz" << std::endl;
...
//                auto dur = end-start;
//                auto msec = std::chrono::duration_cast<std::chrono::milliseconds>(dur).count();
//                std::cout << 1000/msec << " hz" << std::endl;
