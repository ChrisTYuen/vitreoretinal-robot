...
class StoreNode:
    """
    This class saves data from the semi automatic positioning program allowing for logging and making figures.
    """
    def __init__(self):
        ...
        self.data_logger = DataloggerInterface(1000)
    ...
    def _store_callback_patient_side(self, msg):
        ...
        if msg.name == "eyeball_variables":
            ...
            # # Positioning
            # for i, sublist in enumerate(self.parameter.positioning_list):
            #     self.data_logger.log(f"positioning_list_{i+1}", np.array(sublist))
            # for i in range(4):
            #     self.data_logger.log(f"om_trajectory_point_list{i+1}", vec4(self.parameter.om_trajectory_point_list[i]))
            ...
            # self.data_logger.log("instrument_velocity_setup", float(self.parameter.si_velocity_setup))
            # self.data_logger.log("instrument_velocity_planar", float(self.parameter.si_velocity_planar))
            # self.data_logger.log("instrument_velocity_planar_vert", float(self.parameter.si_velocity_planar2))            
            # self.data_logger.log("instrument_velocity_vertical", float(self.parameter.si_velocity_vertical))
            # self.data_logger.log("instrument_velocity_vertical_additional", float(self.parameter.si_velocity_vertical_additional))

            # self.data_logger.log("threshold_overlap_prevention_vrep", float(self.parameter.threshold_overlap_prevention_vrep))
            # self.data_logger.log("threshold_vertical_positioning_vrep", float(self.parameter.threshold_vertical_positioning_vrep))
            # self.data_logger.log("threshold_planar_positioning_pixel", float(self.parameter.threshold_planar_positioning_pixel))
            # self.data_logger.log("threshold_overlap_prevention_pixel", float(self.parameter.threshold_overlap_prevention_pixel))
            # self.data_logger.log("threshold_vertical_positioning_pixel", float(self.parameter.threshold_vertical_positioning_pixel))
            # self.data_logger.log("tip_dis_count", float(self.parameter.tip_dis_count))
            # self.data_logger.log("converter_per_mm_vrep", float(self.parameter.converter_per_mm_vrep))
            # self.data_logger.log("converter_per_mm", float(self.parameter.converter_per_mm))
            # self.data_logger.log("additional_positioning_margin", float(self.parameter.additional_positioning_margin))
            ...
            # self.data_logger.log("n_planar", float(self.parameter.n_planar))
            # self.data_logger.log("n_vertical", float(self.parameter.n_vertical))
            # self.data_logger.log("damping_planar", float(self.parameter.damping_planar))
            # self.data_logger.log("damping_overlap_instrument", float(self.parameter.damping_overlap_instrument))
            # self.data_logger.log("damping_overlap_light", float(self.parameter.damping_overlap_light))
            # self.data_logger.log("damping_vertical_instrument", float(self.parameter.damping_vertical_instrument))
            # self.data_logger.log("damping_vertical_light", float(self.parameter.damping_vertical_light))
            # self.data_logger.log("para_D_velocity", float(self.parameter.D_velocity))
            ...
            self.data_logger.log("d_safe_rcm_om", float(self.parameter.d_safe_rcm_om))
        ...
        # elif msg.name == "additional_depth":
        #     depth = msg.value
        #     self.data_logger.log("additional_depth", float(depth[0]))

        elif msg.name == "thresholds":
            thresholds = msg.value
            self.data_logger.log("threshold_overlap_prevention", float(thresholds[0]))
            self.data_logger.log("threshold_vertical_positioning", float(thresholds[1]))
            ...
            # shaft_distance, tip_distance, planar error, current_sum, current_step, second task velocity
            self.data_logger.log("shaft_tip_distances", np.array([store_data[26:32]]).reshape([6, 1]))

            x1 = DQ(np.array([store_data[32:40]]).reshape([8, 1]))
            x2 = DQ(np.array([store_data[40:48]]).reshape([8, 1]))
            td_si = DQ(np.array([store_data[48:52]]).reshape([4, 1]))
            rcm_si = np.array([store_data[52:56]]).reshape([4, 1])
            rcm_lg = np.array([store_data[56:60]]).reshape([4, 1])
            r_o_e = np.array([store_data[60:64]]).reshape([4, 1])
            forceps_grasp_pos = np.array([store_data[64:67]]).reshape([3, 1])
            ...
            self.store_translations_and_rotations(shadow_t, x1, x2, r2, lz_2, vec4(t1)[3], store_data[30], td_si)

            if self.parameter.orbital_manipulation:
                l1 = normalize(r1 * k_ * conj(r1))
                l2 = normalize(r2 * k_ * conj(r2))
                t_trocar_si, t_trocar_lg = om_kinematics.get_current_rcm_translations(t1, t2, l1, l2, self.eyeball_t, self.eyeball_radius)
                ...
                D_trocar_si = om_kinematics.get_tip_to_moving_trocar_distance_error(t_trocar_si, t1, self.parameter.D_safe_trocar)
                D_trocar_lg = om_kinematics.get_tip_to_moving_trocar_distance_error(t_trocar_lg, t2, self.parameter.D_safe_trocar)
                ...
                self.store_distances(np.hstack([D_rcm, D_eyeball_error, D_collision_error, D_trocar_si, D_trocar_lg, D_tip,
                                                d_rot_plane_1_si, d_rot_plane_1_lg, d_rot_plane_2_si, d_rot_plane_2_lg, d_illumination_cone]))
            else:
                ...
                self.store_distances(np.hstack([D_rcm_error_1, D_rcm_error_2, D_eyeball_error, D_collision_error,
                                                D_trocar_1, D_trocar_2, D_tip, store_data[30]]))
                
                d_second_task = EyeVFI.get_second_task_distance(t1, t2, r1)
                self.data_logger.log("second_task", np.hstack([store_data[30], store_data[31], d_second_task]))  # current_step, second_task_velocity, second_task_distance
    ...
    def store_translations_and_rotations(self, shadow_tip, si_tip_dq, lg_tip_dq, r2, lz_2, si_z, step, td_1):
        """
        store_translations_and_rotations(self, shadow_tip, si_tip_dq, lg_tip_dq, r2, lz_2, si_z, step, td_1) saves
        ...
        translations_and_rotations = np.hstack([vec4(shadow_tip), vec4(si_tip), vec4(lg_tip), i_another, l_another, s_another, vec4(r2), vec4(lz_2), si_z, step, vec4(td_1)])
