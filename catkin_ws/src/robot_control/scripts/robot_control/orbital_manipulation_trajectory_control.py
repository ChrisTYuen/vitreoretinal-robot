        ...
        while point < trajectory_point_num:
            if point == trajectory_point_num - 1:
                ...
                total_iteration = kine_func.get_p2p_iteration(trajectory_point_list[point] * eye.eyeball_radius,
                                                              td_eye, parameter.tau, parameter.setup_velocity)
            elif point == 0:
            ...
            while i < total_iteration:
                ...
                delta_thetas = qp_solver.solve_quadratic_program(2 * (H + parameter.damping * parameter.B_12),
                                                                 c, W, w, np.zeros([1, 13]), np.zeros([1, 1]))

                theta_si = theta_si + delta_thetas[:7] * parameter.tau
                theta_lg = theta_lg + delta_thetas[7:13] * parameter.tau
                theta_si.reshape([7, 1])
                ...
                # Logging
                ##################################
                # store_data = np.hstack([theta_si.T, theta_lg.T, delta_thetas, vec8(x_si), vec8(x_lg), vec8(xd_si), vec8(xd_lg)])
