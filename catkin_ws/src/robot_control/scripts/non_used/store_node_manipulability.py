...
class StoreNode:
    ...
    def __init__(self):
        ...
        self.data_logger = DataloggerInterface(150)
    ...
    def _store_callback_patient_side(self, msg):
        ...
        if msg.name == "eyeball_variables":
            ...
            # Other conditions
            self.data_logger.log("control_target", float(self.parameter.control_target))
            ...
            self.data_logger.log("instrument_velocity_setup", float(self.parameter.setup_velocity))
            ...
            rcm_si = np.array([store_data[58:62]]).reshape([4, 1])
            rcm_lg = np.array([store_data[62:66]]).reshape([4, 1])
            r_o_e = np.array([store_data[66:70]]).reshape([4, 1])
            ...
            self.data_logger.log("td_lg", vec4(td_lg))
            ...
            self.store_translations_and_rotations(x1, x2, r2, lz_2)

            if self.parameter.orbital_manipulation:
                t_trocar_si, t_trocar_lg = om_kinematics.get_current_rcm_translations(t1, t2, r1, r2, self.eyeball_t, self.eyeball_radius)
    ...
    def store_translations_and_rotations(self, si_tip_dq, lg_tip_dq, r2, lz_2):
        ...
        self.data_logger.log("t_si", vec4(si_tip))
        self.data_logger.log("t_lg", vec4(lg_tip))
        translations_and_rotations = np.hstack([i_another, l_another, s_another, vec4(r2), vec4(lz_2)])
    ...
    def store_distances(self, safe_distances):
        self.data_logger.log("constrain_distances", safe_distances)
